...
この workflow は現在停止中です。

再開するまでは、GitHub Actions での preview ビルドは作成されません。

停止理由:

- GitHub Actions の実行コストを抑えるため
- macOS 実機での検証を行わない方針にしたため
- Windows 版はローカル build を優先するため

停止前の構成では、次の preview ビルドを作成していました。
...
ただし現在は `build-preview` job 自体を停止しているため、実行しても build は行われません。
